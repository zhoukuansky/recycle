...
public class OrdersController {
    ...
    public Result userFindOrdersList(@RequestParam(value = "pageNum", defaultValue = "1") Integer pageNum, @RequestParam(value = "pageSize", defaultValue = "10") Integer pageSize, @CurrentUser String[] tokenData) throws Exception {
        Result result = ResultUtil.success();
        int user_c_id = Integer.parseInt(tokenData[1]);
        try {
            result = ResultUtil.success(service.findUserAllOrders(user_c_id, pageNum, pageSize));
        } catch (Exception e) {
            result = handle.exceptionGet(e);
            //result = ResultUtil.error(ExceptionEnum.USER_EXIST);
        }
    ...
    public Result recycleFindOrdersList(@RequestParam(value = "pageNum", defaultValue = "1") Integer pageNum, @RequestParam(value = "pageSize", defaultValue = "10") Integer pageSize, @CurrentUser String[] tokenData) throws Exception {
        Result result = ResultUtil.success();
        int user_b_id = Integer.parseInt(tokenData[1]);
        try {
            result = ResultUtil.success(service.findRecycleAllOrders(user_b_id, pageNum, pageSize));
        } catch (Exception e) {
            result = handle.exceptionGet(e);
            //result = ResultUtil.error(ExceptionEnum.USER_EXIST);
        }
    ...
    public Result OrdersList(@RequestParam(value = "pageNum", defaultValue = "1") Integer pageNum, @RequestParam(value = "pageSize", defaultValue = "10") Integer pageSize, @CurrentUser String[] tokenData) throws Exception {
        Result result = ResultUtil.success();
        //int user_b_id = Integer.parseInt(tokenData[1]);
        int type = Integer.parseInt(tokenData[0]);
        if (type != 3) {
        ...
        try {
            result = ResultUtil.success(service.findAllOrders(pageNum, pageSize));
        } catch (Exception e) {
            result = handle.exceptionGet(e);
            //result = ResultUtil.error(ExceptionEnum.USER_EXIST);
        }
    ...
    @SystemControllerLog(logAction = "deleteOrders", logContent = "超级管理员和回收站可删除订单")
    @ApiOperation(value = "超级管理员和回收站可删除订单", notes = "超级管理员和回收站可删除订单")
    @ApiImplicitParams({
